...
int nearest2(point pt, point cent, int n_cluster, double *d2);
...
inline int
nearest2(point pt, point cent, int n_cluster, double *d2)
{
	int i, min_i;
	point c;
	double d, min_d;
...
#	define for_len for (j = 0, p = pts; j < len; j++, p++)
	...
	for_len p->group = nearest2(p, cent, n_cluster, 0);
	free(d);
}
...
point lloyd(point pts, int len, int n_cluster)
{
	...
	do {
		...
		for_len {
			min_i = nearest2(p, cent, n_cluster, 0);
			if (min_i != p->group) {
				changed++;
				p->group = min_i;
			}
